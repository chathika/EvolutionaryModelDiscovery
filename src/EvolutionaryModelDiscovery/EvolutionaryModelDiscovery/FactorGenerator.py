...
from .Util import *
# Uses NetLogo EMD entry-point annotations as follows:
...
class FactorGenerator:
    ...
    def __init__(self) -> None:
        ...
        to the NetLogo annotations read from the input model.
    ...
    def read_netlogo_function_file(self, factor_file_path: str) -> None:
        '''
                ...
                if not factor_identified:
                    if "@emd" in line and ("@factor" in line or "@operator" in line) and not ("@evolvenextline" in line):
                        ...
                        for emd_parameter in emd_parameters:
                            if "return-type=" in emd_parameter:
                                ...
                                raise Exception("Invalid EMD annotation argument {1} at line {0}.".format(
                    ...
                    if operator_identified:
                        if "-" in parameter_contributions_to_fitness:
                            ...
                            )] = [-1 if sgn == "-" else 1 for sgn in parameter_contributions_to_fitness]
    ...
    def write_class_names(self) -> None:
        ...
        with open(self.model_factor_path, "a+") as f:
            ...
            classNamesString = classNamesString[:(
                len(classNamesString)-1)] + "]"
            f.write(classNamesString)

    def write_python_class_from_factor(self, factor: Factor) -> None:
        '''
        Compile the NetLogo procedure string for the Factor into a Python 
        ...
        for parameterType in factor.get_parameter_types():
            parameterString = parameterString + \
                "{0}, ".format((parameterType + str(occurances)))
        ...
        for parameterType in factor.get_parameter_types():
            netlogoString = netlogoString + \
                '(" + str({0}) + ") '.format((parameterType + str(occurances)))
            occurances = occurances + 1
        netlogoString = netlogoString + '"'
