...
class Visualizer_for_surrogates(object):
    ...
    def __init__(self, fanova):
        ...
        #NGUYEN: for surrogates
        self.surrogate_param_convert_list = {'nbIterations':r'$N^{iters}$', 'mu':r'$\mu$', 'firstTest':r'$T^{first}$', 'minNbSurvival':r'$N^{min}$', 'confidence':'confidence_level', 'softRestart':'enable_soft_restart', 'elitist':'enable_elitist', 'elitistInstances':'elitist_instances', 'testType':'test_type'}
    ...
    def create_all_plots(self, directory, **kwargs):
        ...
        file_format = "png" #'eps'
        text_size = 10 #20, 15
        x_axis_title = False #False

        #categorical parameters
        for param_name in self._fanova.get_config_space().get_categorical_parameters():
            plt.clf()
            outfile_name = os.path.join(directory, param_name.replace(os.sep, "_") + "." + file_format) #NGUYEN: add file_format
            print("creating %s" % outfile_name)
            self.plot_categorical_marginal(param_name, text_size=text_size, x_axis_title=x_axis_title) #NGUYEN: add the last two params
            plt.savefig(outfile_name, format = file_format, bbox_inches='tight') #NGUYEN: add the last two params
        ...
        for param_name in params_to_plot:
            ...
            outfile_name = os.path.join(directory, param_name.replace(os.sep, "_") + "." + file_format) #NGUYEN: add file_format
            print("creating %s" % outfile_name)
            self.plot_marginal(param_name, text_size=text_size, x_axis_title=x_axis_title) #NGUYEN: add the last two params
            plt.savefig(outfile_name, format = file_format, bbox_inches='tight') #NGUYEN: add the last two params
    ...
    def plot_categorical_marginal(self, param, text_size = 10, x_axis_title = True):
        ...
        if rotate_xtick:
            ...
            plt.setp(gca().get_xticklabels(), fontsize=text_size)
            plt.setp(gca().get_yticklabels(), fontsize=text_size)
        ...
        plt.ylabel("Cost", fontsize = text_size)
        if x_axis_title == True:
    ...
    def NGUYEN_plot_pairwise_marginal_oneCat_onePlot(self, directory, param_1, param_2, lower_bound_1=0, upper_bound_1=1, lower_bound_2=0, upper_bound_2=1, resolution=20):
        ...
        if (param_name_1 not in self._fanova.get_config_space().get_categorical_parameters()) and (param_name_2 in self._fanova.get_config_space().get_categorical_parameters()):
            ...
            for categorical_id in range(categorical_size):
                ...
                lower_curve = mean - std
                upper_curve = mean + std
                legend_label = self.surrogate_param_convert_list[param_name_2] + ' = ' + categorical_val
            ...
            print("creating %s" % outfile_name)
            plt.xlabel(self.surrogate_param_convert_list[param_1])
            plt.ylabel("Cost")
            #plt.legend()
            lgd = plt.legend(loc=9, bbox_to_anchor=(0.5, 1.3), ncol=1)
    ...
    def NGUYEN_plot_pairwise_marginal_twoCats(self, directory, param_1, param_2, lower_bound_1=0, upper_bound_1=1, lower_bound_2=0, upper_bound_2=1, resolution=20):
        ...
        if (param_name_1 in self._fanova.get_config_space().get_categorical_parameters()) and (param_name_2 in self._fanova.get_config_space().get_categorical_parameters()):
            ...
            ls_categorical_vals_1.insert(0, ' ')
            print(ls_categorical_vals_1)
            #plt.gca().set_xticklabels(ls_categorical_vals_1.insert(0,''),rotation=45, ha="right")
    ...
    def plot_pairwise_marginal(self, param_1, param_2, lower_bound_1=0, upper_bound_1=1, lower_bound_2=0, upper_bound_2=1, resolution=20):
        ...
        ax.set_xlabel(self.surrogate_param_convert_list[param_name_1])
        ax.set_ylabel(self.surrogate_param_convert_list[param_name_2])
        ax.set_zlabel("Cost")
    ...
    def plot_marginal(self, param, lower_bound=0, upper_bound=1, is_int=False, resolution=100, log_scale=False, text_size = 10, x_axis_title = True):
        ...
        if log_scale or (np.diff(display_grid).std() > 0.000001 and param_name in self._fanova.get_config_space().get_continuous_parameters()):
            ...
            plt.plot(display_grid, mean, 'b')
        plt.fill_between(display_grid, upper_curve, lower_curve, facecolor='yellow', alpha=0.6) #NGUYEN: change color from red to yello
        
        plt.setp(gca().get_xticklabels(), fontsize=text_size)
        plt.setp(gca().get_yticklabels(), fontsize=text_size)
        
        if x_axis_title == True:
            plt.xlabel(param_name, fontsize = text_size)
